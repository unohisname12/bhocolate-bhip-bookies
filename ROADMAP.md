...
This structure decision was made after additional assets were created.

Blue Koala now has 4 behavior groups:
- `adult-walk` (legacy existing in-game location)
- `hatching`
- `dancing`
- `cleaning`
...
Future characters should follow this same behavior-group folder system and staging flow (`/game ready/` -> organized folders -> config integration).

CURRENT EXECUTION STATE
CURRENT STEP
Step 5: Game Engine Extraction

Timestamp Format
Use local time in this exact format when a step is completed:
`YYYY-MM-DD HH:MM:SS TZ`

Completed Work
- [x] Step 0 completed — finished at `2026-03-30 19:36:32 PDT`
- [x] Step 1 completed — finished at `2026-03-30 19:36:32 PDT`
- [x] Step 2 completed — finished at `2026-03-30 19:36:32 PDT`
- [x] Step 3 completed — finished at `2026-03-30 20:38:34 PDT`
- [x] Step 4 completed — finished at `2026-03-30 21:05:02 PDT`

Step Completion Log
- Step 0: `2026-03-30 19:36:32 PDT`
- Step 1: `2026-03-30 19:36:32 PDT`
- Step 2: `2026-03-30 19:36:32 PDT`
- Step 3: `2026-03-30 20:38:34 PDT`
- Step 4: `2026-03-30 21:05:02 PDT`

Step 0 Completed Tasks
- [x] Audited build/lint/typecheck baseline
- [x] Added dual-state ownership documentation in src/App.tsx
- [x] Verified dead Vite assets were already removed (src/assets/react.svg, src/assets/vite.svg)
- [x] Verified root zip artifact was absent
- [x] Kept src/services/firebase/config.ts and marked as placeholder for Step 14

Step 1 Completed Tasks
- [x] Updated src/types/pet.ts with PetStage, PetMood, PetNeeds, PetStats, PetProgression, Egg/EggState
- [x] Updated src/types/player.ts with QuizOutcome, required currencies.coins, quizOutcome: QuizOutcome | null
- [x] Updated src/types/session.ts with ScreenName and EggInteractionState
- [x] Updated src/types/events.ts with GameEventType literal union
- [x] Updated src/types/index.ts re-exports and GameScreen alias
- [x] Updated src/types/engine.ts and re-export bridge in src/engine/core/EngineTypes.ts
- [x] Fixed dependent factories/services/screens for new required fields
- [x] Validation passed: npx tsc --noEmit and npm run build

Step 2 Completed Tasks
- [x] Added consolidated constants in src/config/gameConfig.ts: DECAY_RATES, TICK_INTERVAL_MS, DECAY_INTERVAL_MS, PET_STATE_THRESHOLDS, ANIMATION_DEFAULTS, EGG_CONFIG
- [x] Added streak placeholder in src/config/rewardConfig.ts
- [x] Normalized species definitions with assetKey alignment and added koala entry in src/config/speciesConfig.ts
- [x] Typed src/config/assetManifest.ts with AssetManifest, BaseAssetConfig, SpriteSheetConfig, AnimationRange
- [x] Added config barrel + validateConfigs() in src/config/index.ts
- [x] Wired integrations:
- [x] careEngine.ts uses DECAY_RATES
- [x] petStateMachine.ts and MoodSystem.ts use PET_STATE_THRESHOLDS
- [x] PetNeedSystem.ts uses DECAY_RATES
- [x] PetSprite.tsx uses ANIMATION_DEFAULTS.scale
- [x] useGameEngine.ts uses TICK_INTERVAL_MS
- [x] useGameState.ts uses DECAY_INTERVAL_MS
- [x] App.tsx calls validateConfigs() in dev mode on mount
- [x] Validation passed: npx tsc --noEmit and npm run build

Step 3 Completed Tasks
- [x] Step 3.A Created src/devtools/devToolsActions.ts with DEV_* action definitions
- [x] Extended GameEngineAction union in src/engine/core/ActionTypes.ts to include DEV_* actions
- [x] Added DEV_* reducer handling in src/engine/state/engineReducer.ts (needs, force state, jump screen, deep state patch)
- [x] Extended GameEngine in src/engine/core/GameEngine.ts with setTickInterval(), getActionLog(), onAction()
- [x] Created src/devtools/NeedSliders.tsx and wired DEV_SET_NEEDS action
- [x] Created src/devtools/TimeControl.tsx and wired pause/play/step/2x/10x controls
- [x] Created src/devtools/DevToolsOverlay.tsx with Ctrl+Shift+D toggle and z-index 9999
- [x] Created src/devtools/StateInspector.tsx (state view/edit + DEV_SET_STATE, DEV_FORCE_PET_STATE, DEV_JUMP_SCREEN)
- [x] Created src/devtools/ActionLog.tsx (last 100 actions, filter, payload expand)
- [x] Created src/devtools/SpriteDebugger.tsx (frame scrub, play/pause, animation-aware debug)
- [x] Created src/devtools/SnapshotManager.tsx (save/load/delete/export/import session snapshots)
- [x] Added devtools exports in src/devtools/index.ts
- [x] Wired dev-only overlay render in src/App.tsx
- [x] Removed moved manual sprite/need debug controls from src/screens/TestModeScreen.tsx; kept SpriteAutoAssist workspace
- [x] Validation passed: npx tsc --noEmit and npm run build

Step 4 Completed Tasks
- [x] Created `src/engine/animation/types.ts` with `SpriteSheetConfig`, `AnimationRange`, `AnimationName`, `SpriteRenderStyle`
- [x] Created `src/engine/animation/SpriteRenderer.ts` with pure `computeSpriteStyle()`
- [x] Created `src/engine/animation/AnimationController.ts` with validated ranges and deterministic frame progression
- [x] Rebuilt `src/components/pet/PetSprite.tsx` to use new API: `speciesId`, `animationName`, `scale`, `paused`, `onFrameChange`
- [x] Removed all sprite auto-detection/setInterval logic and switched to `requestAnimationFrame`
- [x] Updated `src/config/assetManifest.ts` so every pet has explicit sprite sheet config + full animation ranges
- [x] Updated `src/screens/PetHomeScreen.tsx` and `src/screens/TestModeScreen.tsx` to new `PetSprite` API
- [x] Removed old `AnimationSystem` usage from `src/engine/state/engineReducer.ts`
- [x] Updated `src/utils/spriteAutoAssist.tsx` typing to use `SpriteSheetConfig`
- [x] Updated `SPRITE_SYSTEM_RULES.md` for config-driven/no-auto-detection architecture
- [x] Validation passed: npx tsc --noEmit and npm run build

Next Task
Step 5.A: Rewrite src/engine/core/GameEngine.ts as pure engine class
...
Open app → initial state loads → quiz screen (or incubation if no quiz yet)
Incubate egg → tap to progress → hatch
Pet appears with correct species sprite
Needs decay over time → feed, play, clean, heal
Open math screen → solve problems → earn tokens → streak builds
Open shop → buy food → check inventory → use item
Decorate room → mood bonus applies
Pet levels up → evolution available → evolve
Start battle → math bonus round → win → rewards
Check achievements → verify unlocks
Save game → refresh → state persists
Export save → clear game → import save → state restored
Open devtools → manipulate state → verify all tools work
Build for production → deploy → Lighthouse audit
Install as PWA → play offline
Add Comment