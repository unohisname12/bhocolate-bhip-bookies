...
import type { ShopItemCategory } from '../config/shopConfig';
...
export const ShopScreen: React.FC<ShopScreenProps> = ({ tokens, coins, mpLifetime, dispatch, onClose }) => {
  ...
  const visibleTierItems = SHOP_ITEMS.filter(i => i.requiredMPTier).filter(i => i.requiredMPTier === 'silver' && playerTier === 'silver');
  const hasNewRef = useRef(false);
  hasNewRef.current = visibleTierItems.some(i => !seenItems.has(i.id));

  // Mark tier items as seen after a short delay so the "NEW" badge is visible on first open
  useEffect(() => {
    if (!hasNewRef.current) return;
    const timer = setTimeout(() => {
      const updated = new Set(seenItems);
      visibleTierItems.forEach(i => updated.add(i.id));
  ...
  const visibleItems = SHOP_ITEMS.filter(item => {
    if (!item.requiredMPTier) return true;
    return item.requiredMPTier === 'silver' && playerTier === 'silver';
  });
    ...
    <div className="flex flex-col min-h-screen bg-slate-900 w-full max-w-lg mx-auto p-4 pb-20">
      ...
      <div className="grid grid-cols-2 gap-3">
        {filtered.map((item) => {
          ...
          const canAfford = tokenCost > 0 ? tokens >= tokenCost : coins >= coinCost;
          const isNew = item.requiredMPTier && !seenItems.has(item.id);
          ...
          return (
            <div
              ...
              } ${!isNew && canAfford ? 'cursor-pointer' : ''}`}
              onClick={() => canAfford && handleBuy(item.id, tokenCost, coinCost)}
            >
              {isNew && (
              ...
              <GameIcon icon={item.icon} size="w-12 h-12" className="text-4xl mb-2 drop-shadow-md" alt={item.name} />
              <span className="font-bold text-slate-200 text-sm mb-1">{item.name}</span>
              <p className="text-xs text-slate-400 text-center mb-2">{item.description}</p>
              <div className="flex gap-2 text-xs font-black">
                ...
                {coinCost > 0 && (
                  <span className={`flex items-center gap-0.5 ${canAfford ? 'text-cyan-400' : 'text-slate-500'}`}>
