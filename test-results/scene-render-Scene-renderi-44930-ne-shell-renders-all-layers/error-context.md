...
- Name: scene-render.spec.ts >> Scene rendering with pet >> game scene shell renders all layers
- Location: e2e/scene-render.spec.ts:67:3
...
Locator: locator('.fixed.inset-0.overflow-hidden')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.fixed.inset-0.overflow-hidden')
...
```ts
  ...
  2   | 
  3   | // Seed a saved game state so we land on the home screen with a pet.
  4   | // Version 4 matches the new 2-room system.
  5   | const SEED_STATE = {
  6   |   version: 4,
  ...
  23  |       bond: 10,
  24  |       bornAt: Date.now(),
  25  |       needs: { health: 80, hunger: 70, happiness: 90, cleanliness: 60 },
  26  |       progression: { level: 2, xp: 30 },
  27  |       moves: [
